export const treatments = [
  { 
    id: 'osteopathy', 
    title: 'Osteopathy', 
    desc: 'Holistic manual therapy focusing on the body\'s natural ability to heal itself. Effective for back pain, joint problems, and headaches.', 
    image: 'https://osteopathyandwellbeing.co.uk/wp-content/uploads/2018/02/4590922.jpg',
    longDesc: 'Osteopathy in Herne Bay is a great way to address musculoskeletal issues. With its holistic approach, osteopathy focuses on the body\'s natural ability to heal itself. Whether you\'re dealing with back pain, joint problems, or headaches, an osteopath in Herne Bay can provide personalised treatment to help you feel better. Book an appointment today and experience the benefits of osteopathy firsthand! While osteopaths are best known for their treatment of back pain, osteopathy can also help with a wide range of other musculoskeletal conditions.'
  },
  { 
    id: 'swedish-massage', 
    title: 'Swedish Massage', 
    desc: 'Luxurious massage to help the lymph system flush metabolic wastes, shortening recovery time.', 
    image: 'https://osteopathyandwellbeing.co.uk/wp-content/uploads/2018/02/4590924.jpg',
    longDesc: 'If you’re suffering from muscular strain, Swedish massage can help your body’s lymph system flush metabolic wastes such as lactic acid and uric acid from your muscles. This shortens recovery time and helps you feel better faster.'
  },
  { 
    id: 'physiotherapy', 
    title: 'Physiotherapy', 
    desc: 'Restores movement and function affected by injury, illness or disability.', 
    image: 'https://osteopathyandwellbeing.co.uk/wp-content/uploads/2018/02/foot.jpg',
    longDesc: 'Physiotherapy helps to restore movement and function when someone is affected by injury, illness or disability. It can also help to reduce your risk of injury or illness in the future.'
  },
  { 
    id: 'acupuncture', 
    title: 'Acupuncture', 
    desc: 'Western-style medical acupuncture stimulating sensory nerves to produce natural pain-relieving substances.', 
    image: 'https://osteopathyandwellbeing.co.uk/wp-content/uploads/2018/03/needles.jpg',
    longDesc: 'Traditional Chinese acupuncture is based on the belief that it can restore the flow of Qi, an energy that flows through your body, while western medical acupuncture is evidence-based and is only administered after a full diagnosis.'
  }
];
